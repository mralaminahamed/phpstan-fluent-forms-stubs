#!/usr/bin/env bash
#
# Generate Fluent Forms stubs for the latest versions.
#
# This script:
# 1. Fetches the latest Fluent Forms versions from WordPress.org
...
while IFS= read -r VERSION; do
    ...
    if git diff-index --quiet HEAD --; then
        ...
        git add .
        git commit -m "Generate stubs for Fluent Forms ${VERSION}"
        git tag "v${VERSION}"
    fi
