#!/usr/bin/env bash
#
# Generate Fluent Forms stubs for all available versions.
#
# This script:
# 1. Fetches all available Fluent Forms versions from WordPress.org
...
OUTPUT_FILE="$ROOT_DIR/fluentform_versions.txt"
...
while IFS= read -r VERSION; do
    ...
    git commit -m "Generate stubs for Fluent Forms ${VERSION}" || true
    git tag "v${VERSION}"
